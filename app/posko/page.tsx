...
export default function SurveyPage() {
  ...
  // Validasi
  const validateForm = () => {
    if (!form.nm_posko || !form.id_desa || !form.id_kecamatan) {
  ...
  const exportToExcel = () => {
    const dataToExport = surveys.map((s) => ({
      nm_posko: s.nm_posko,
      Kecamatan: s.kecamatan?.nm_kecamatan || "",
      Desa: s.desa?.nm_desa || "",
    }));
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.id_posko) return toast({ title: "❌ ID tidak ditemukan" });
    ...
    try {
      ...
      const res = await fetch(`/api/posko/${form.id_posko}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
  ...
  // Delete
  const handleDelete = async (id_posko: number) => {
    if (!confirm("Yakin ingin menghapus data ini?")) return;
    try {
      const res = await fetch(`/api/posko/${id_posko}`, { method: "DELETE" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Gagal menghapus data");

      toast({ title: "🗑️ Data berhasil dihapus" });
      setSurveys((prev) => prev.filter((s) => s.id_posko !== id_posko));
    } catch (err: any) {
      toast({ title: err.message });
    }
  };
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        <main className="p-6 space-y-8">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold">📋 Data Posko</h2>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent className="sm:max-w-screen-lg w-full rounded-xl shadow-2xl p-0 overflow-hidden">
                ...
                <ScrollArea className="max-h-[75vh] px-6 py-4">
                  <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4" aria-label="Form tambah posko">
                    {/* Informasi Umum */}
                    <Card className="col-span-full p-4 border border-gray-200 shadow-sm">
                      <h3 className="font-semibold text-gray-700 mb-3">📋 Informasi Umum</h3>
                       <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
                        <InputField id="nm_posko" label="Nama Posko" value={form.nm_posko} onChange={(e) => setForm({ ...form, nm_posko: e.target.value })} required />
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Nama Posko</TableHead>
                  ...
                  <TableBody>
                    {paginatedSurveys.map((s, index) => (
                      <TableRow key={s.id_posko}>
                         <TableCell className="text-center">{index + 1}</TableCell>
                        <TableCell>{s.nm_posko}</TableCell>
                        ...
                        <TableCell className="flex gap-2">
                          <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => {
                            ...
                            <Button size="sm" variant="destructive" onClick={() => handleDelete(s.id_posko)}>🗑️ Hapus</Button>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
          ...
          <Dialog open={mapModal} onOpenChange={setMapModal}>
            <DialogContent className="max-w-3xl">
              <DialogHeader>
                <DialogTitle>Data Posko</DialogTitle>
              </DialogHeader>
          ...
          <Dialog open={editOpen} onOpenChange={setEditOpen}>
            <DialogContent className="sm:max-w-screen-lg w-full rounded-xl shadow-2xl p-0 overflow-hidden">
              <DialogHeader className="border-b px-6 py-4 bg-gray-50">
                <DialogTitle className="text-xl font-semibold text-gray-800">Edit Data Posko</DialogTitle>
              </DialogHeader>
              ...
              <ScrollArea className="max-h-[75vh] px-6 py-4">
                <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Card className="col-span-full p-4 border border-gray-200 shadow-sm">
                    <h3 className="font-semibold text-gray-700 mb-3">📋 Informasi Umum</h3>
                    <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
                      <InputField id="editnm_posko" label="Nama Posko" value={form.nm_posko} onChange={(e) => setForm({ ...form, nm_posko: e.target.value })} required />
